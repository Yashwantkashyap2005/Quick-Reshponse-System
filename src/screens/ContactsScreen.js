  ...
  const confirmDelete = (contactId) => {
    if (Platform.OS === 'web') {
        ...
        { text: 'Delete', style: 'destructive', onPress: async () => {
            try { await deleteContact(user.uid, contactId); } catch (e) { console.error(e); }
        }}
        ]);
    }
  };
  ...
  return (
    <View style={styles.bg}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.contactCard} onPress={() => openModal(item)} onLongPress={() => confirmDelete(item.id)} delayLongPress={600}>
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalBg}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholder="Relation (e.g. Parent, Friend)" placeholderTextColor="#888" value={relation} onChangeText={setRelation} />
...
const styles = StyleSheet.create({
  ...
  addBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#6C63FF22', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#6C63FF44' },
  headerTitle: { fontSize: 18, fontWeight: '700', color: '#fff' },
  list: { paddingHorizontal: 20, gap: 12, paddingBottom: 40 },
  contactCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#13132A', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#2A2A3C' },
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#6C63FF33', alignItems: 'center', justifyContent: 'center', marginRight: 14, borderWidth: 1.5, borderColor: '#6C63FF55' },
  ...
  contactRelation: { fontSize: 11, color: '#6C63FF', fontWeight: '600', marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.8 },
  actionRow: { flexDirection: 'row', gap: 8 },
  actionBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: '#4ADE8022', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#4ADE8044' },
  emptyContainer: { alignItems: 'center', marginTop: 80, gap: 12 },
  emptyText: { color: '#444', fontSize: 16 },
  modalBg: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', padding: 20 },
  modalContent: { backgroundColor: '#13132A', borderRadius: 20, padding: 24, borderWidth: 1, borderColor: '#2A2A3C' },
  modalTitle: { fontSize: 20, fontWeight: '700', color: '#fff', marginBottom: 20, textAlign: 'center' },
  input: { backgroundColor: '#1A1A2E', borderRadius: 12, padding: 14, color: '#fff', fontSize: 15, marginBottom: 12, borderWidth: 1, borderColor: '#2A2A3C' },
